...
export default function ProfilePage({ onBack }: ProfilePageProps) {
    ...
    useEffect(() => {
        const loadTeamData = async () => {
            ...
            try {
                ...
                const teamPersonas = await fetchPersonas(user.id, currentTeam?.id || null, token);
    ...
    const loadPersonas = async () => {
        ...
        if (user) {
            const { data: { session } } = await supabase.auth.getSession();
            const list = await fetchPersonas(user.id, currentTeam?.id || null, session?.access_token);
            setPersonas(list);
        }
    };
    ...
    const handleSavePersona = async () => {
        ...
        try {
            ...
            if (cleanData.id) {
                ...
                saved = await saveUserPersona(cleanData, token);
            }
