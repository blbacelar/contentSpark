...
export default function AuthPage() {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    if (!isSupabaseConfigured) {
      setError('App configuration missing: set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY in your .env file.');
      return;
    }
    ...
    try {
      ...
      if (isSignUp) {
        ...
        if (signInError) {
          // If verification is required, this is expected and user should see success guidance.
    ...
    <div className="min-h-screen bg-[#F2F2F2] flex items-center justify-center p-4">
      <div className="bg-white rounded-[32px] w-full max-w-md shadow-2xl overflow-hidden animate-scale-in border border-white p-8">
        ...
        {!isSupabaseConfigured && (
          <div className="bg-amber-50 text-amber-800 px-4 py-3 rounded-xl text-xs font-bold mb-6 border border-amber-200 flex items-start gap-2">
            <div className="mt-0.5 min-w-[4px] h-[4px] rounded-full bg-amber-700" />
            Missing Supabase env vars. Add VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY to run authentication.
          </div>
        )}
