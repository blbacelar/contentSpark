...
export const createTeam = async (name: string, userId: string, token?: string | null): Promise<Team | null> => {
    try {
        ...
        const isConflict = message.includes('Supabase API Error: 409');
        if (isConflict || message.includes('unique_personal_team_per_user') || message.includes('duplicate key value')) {
            const existing = await supabaseFetch(`teams?owner_id=eq.${userId}&select=id,name,owner_id,created_at,invitation_code&limit=1`, {
                method: 'GET'
            }, token || undefined);
            if (existing?.[0]) {
