...
import { generateId, updateContent, deleteContent, generateContent, fetchTeamBranding } from '../services/genai';
...
export function useIdeaManagement({
    ...
    const updateIdeaWrapper = async (updated: ContentIdea) => {
        ...
        if (user) {
            try {
                ...
                const savedIdea = await updateContent({
                    id: updated.id,
                    date: updated.date,
                    time: updated.time,
                    ...
                    canva_prompt: updated.canva_prompt
                }, user.id, session?.access_token);

                if (savedIdea?.id && savedIdea.id !== updated.id) {
                    setIdeas(prev => prev.map(i => {
                        if (i.id !== updated.id) return i;
                        return {
                            ...i,
                            ...savedIdea,
                            platform: Array.isArray((savedIdea as any).platform)
                                ? (savedIdea as any).platform
                                : i.platform
                        } as ContentIdea;
                    }));
                }
    ...
    const validateAndGenerate = async (userPersona: PersonaData | null, allPersonas: PersonaData[]) => {
    ...
    const performGeneration = async (specificPersona: PersonaData) => {
        ...
        try {
            let effectiveBranding = profile?.branding;

            if (currentTeam?.id && session?.access_token) {
                try {
                    const teamBranding = await fetchTeamBranding(currentTeam.id, session.access_token);
                    if (teamBranding) {
                        effectiveBranding = teamBranding;
                    }
                } catch (brandingErr) {
                    // Non-blocking: fallback to profile branding if team branding fetch fails.
                    console.warn('Failed to fetch team branding for generation. Falling back to profile branding.', brandingErr);
                }
            }
