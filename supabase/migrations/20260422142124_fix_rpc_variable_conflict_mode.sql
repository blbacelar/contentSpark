-- Resolve PL/pgSQL identifier ambiguity when argument names overlap table columns.

DROP FUNCTION IF EXISTS public.create_team_with_owner(text, uuid);

CREATE FUNCTION public.create_team_with_owner(name text, user_id uuid)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
	...
	new_code := substr(md5(random()::text || clock_timestamp()::text || p_user_id::text), 1, 12);
